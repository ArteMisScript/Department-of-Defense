script_name("DefenseHelper")
script_authors("Jack Cortez | Harry Cortez")
script_description("Скрипт для Department of Defense Arizona Saint Rose and Glengale")
...
if not doesFileExist(getGameDirectory().."/SAMPFUNCS.asi") then
	...
	ffi.C.MessageBoxA(0, sampfuncsNot, "DefenseHelper", 0x00000030 + 0x00010000) 
end
if #nofiles > 0 then
	ffi.C.ShowWindow(ffi.C.GetActiveWindow(), 6)
	ffi.C.MessageBoxA(0, errorText:format(table.concat(nofiles, "\n\t\t")), "DefenseHelper", 0x00000030 + 0x00010000) 
end
...
chgName.org = {u8"Армия ЛС", u8"ВМС", u8"Тюрьма ЛВ"}
chgName.rank = {u8"Рядовой", u8"Капрал", u8"Мастер-сержант", u8"Сержант-майор", u8"Уорент-Офицер 1 класса", u8"Капитан", u8"Майор", u8"Подполковник", u8"Полковник", u8"Генерал", u8"Министр Обороны"}

local list_org_BL = {"Армия ЛС", "ВМС", "Тюрьма ЛВ"} 
local list_org	= {u8"Армия ЛС", u8"ВМС", u8"Тюрьма ЛВ"}
local list_org_en = {"Army of Los Santos","Army of San-Fierro","Maximum Security Prison of Las Venturas"}
local list_sex	= {fa.ICON_MALE .. u8" Мужской", fa.ICON_FEMALE .. u8" Женский"} --ICON_MALE ICON_FEMALE 
local list_rank	= {u8"Рядовой", u8"Капрал", u8"Мастер-сержант", u8"Сержант-майор", u8"Уорент-Офицер 1 класса", u8"Капитан", u8"Майор", u8"Подполковник", u8"Полковник", u8"Генерал", u8"Министр Обороны"}
--chat
...
function imgui.BeforeDrawFrame()
  if fa_font == nil then
    ...
    fa_font = imgui.GetIO().Fonts:AddFontFromFileTTF('moonloader/DefenseHelper/files/font-icon.ttf', 15.0, font_config, fa_glyph_ranges)
  end
end
...
function main()
	...
	if script.this.filename:find("%.luac") then
		os.rename(getWorkingDirectory().."\\DefenseHelper.luac", getWorkingDirectory().."\\DefenseHelper.lua") 
	end
--	repeat wait(100) until sampIsLocalPlayerSpawned()
	------------
	...
	------------
		...
		if not doesFileExist(dirml.."/DefenseHelper/files/logo-DefenseHelper.png") then print("{FF2525}Ошибка: {FFD825}Отсутствует изображение logo-DefenseHelper.png") end
		if not  doesFileExist(dirml.."/DefenseHelper/files/discord-logo.png") then print("{FF2525}Ошибка: {FFD825}Отсутствует изображение discord-logo.png") end
		if not  doesFileExist(dirml.."/DefenseHelper/files/discord-site.png") then print("{FF2525}Ошибка: {FFD825}Отсутствует изображение discord-site.png") end
		if not  doesFileExist(dirml.."/DefenseHelper/files/discord-role.png") then print("{FF2525}Ошибка: {FFD825}Отсутствует изображение discord-role.png") end
		if not  doesFileExist(dirml.."/DefenseHelper/files/discord-med.png") then print("{FF2525}Ошибка: {FFD825}Отсутствует изображение discord-med.png") end
		if not  doesFileExist(dirml.."/DefenseHelper/files/discord-nick.png") then print("{FF2525}Ошибка: {FFD825}Отсутствует изображение discord-nick.png") end
		logoMH = imgui.CreateTextureFromFile(dirml.."/DefenseHelper/files/logo-DefenseHelper.png") 
		logoDis = imgui.CreateTextureFromFile(dirml.."/DefenseHelper/files/discord-logo.png")
		
		--Проверка на существование папкок
		if not doesDirectoryExist(dirml.."/DefenseHelper/files/") then
			print("{F54A4A}Ошибка. Отсутствует папка. {82E28C}Создание папки под файлы")
			createDirectory(dirml.."/DefenseHelper/files/")
		end
		if not doesDirectoryExist(dirml.."/DefenseHelper/Binder/") then
			print("{F54A4A}Ошибка. Отсутствует папка. {82E28C}Создание папки для биндера.")
			createDirectory(dirml.."/DefenseHelper/Binder/")
		end
		if not doesDirectoryExist(dirml.."/DefenseHelper/Шпаргалки/") then
			print("{F54A4A}Ошибка. Отсутствует папка. {82E28C}Создание папки для шпор")
			createDirectory(dirml.."/DefenseHelper/Шпаргалки/")
		end
		if not doesDirectoryExist(dirml.."/DefenseHelper/Департамент/") then
			print("{F54A4A}Ошибка. Отсутствует папка. {82E28C}Создание папки для новостей в департамент")
			createDirectory(dirml.."/DefenseHelper/Департамент/")
		end
		--Загрузка файла настроек
		if doesDirectoryExist(dirml.."/DefenseHelper/Департамент/") then
			getGovFile()
		end
		if doesFileExist(dirml.."/DefenseHelper/MainSetting.med") then
		print("{82E28C}Чтение настроек...")
		local f = io.open(dirml.."/DefenseHelper/MainSetting.med")
			local setf = f:read("*a")
			f:close()
			...
			if res and type(set) == "table" then 
				...
				os.remove(dirml.."/DefenseHelper/MainSetting.med")
		...
		if not cb_imageDis.v then
			disSite = imgui.CreateTextureFromFile(dirml.."/DefenseHelper/files/discord-site.png")
			disRole = imgui.CreateTextureFromFile(dirml.."/DefenseHelper/files/discord-role.png")
			disMed = imgui.CreateTextureFromFile(dirml.."/DefenseHelper/files/discord-med.png")
			disNick = imgui.CreateTextureFromFile(dirml.."/DefenseHelper/files/discord-nick.png")
		end

	print("{82E28C}Чтение настроек команд...")
	if doesFileExist(dirml.."/DefenseHelper/cmdSetting.med") then
	--register cmd
		local f = io.open(dirml.."/DefenseHelper/cmdSetting.med")
		...
		if res and type(keys) == "table" then
			...
			os.remove(dirml.."/DefenseHelper/cmdSetting.med")
		end
	else
	...
	if doesFileExist(dirml.."/DefenseHelper/bindSetting.med") then
		local f = io.open(dirml.."/DefenseHelper/bindSetting.med")
		...
		if res and type(list) == "table" then
			...
			os.remove(dirml.."/DefenseHelper/bindSetting.med")
	...
	lockPlayerControl(false)
		...
		sampRegisterChatCommand("mo", function() mainWin.v = not mainWin.v end)
		...
		sampRegisterChatCommand("kpp", funCMD.kpp)
		...
		sampRegisterChatCommand("mo-delete", funCMD.del)
		sampAddChatMessage("{FFFFFF}[{EE4848}DefenseHelper{FFFFFF}]: Скрипт инициализирован.", 0xEE4848)
		repeat wait(100) until sampIsLocalPlayerSpawned()
		_, myid = sampGetPlayerIdByCharHandle(PLAYER_PED)
		myNick = sampGetPlayerNickname(myid)
		
		sampAddChatMessage(string.format("{FFFFFF}[{EE4848}DefenseHelper{FFFFFF}]: Приветствую, %s. Для активации главного меню пропишите в чат {22E9E3}/mo.", sampGetPlayerNickname(myid):gsub("_"," ")), 0xEE4848)
		wait(200)
		if buf_nick.v == "" then 
			sampAddChatMessage("{FFFFFF}[{EE4848}DefenseHelper{FFFFFF}]: Похоже у тебя не настроена основная информация. ", 0xEE4848)
			sampAddChatMessage("{FFFFFF}[{EE4848}DefenseHelper{FFFFFF}]: Зайди в главном меню в раздел \"Настройки\" и настрой себе всё по \"фэн-шую\".", 0xEE4848)
		end
		autoupdate("https://raw.githubusercontent.com/ArteMisScript/Department-of-Defense/main/version.json", '['..string.upper(thisScript().name)..']: ', "https://github.com/ArteMisScript/Department-of-Defense/blob/main/DefenseHelper.luac?raw=true")
  while true do
	wait(0)
...
function mainSet()
		...
		imgui.PushItemWidth(278);
			imgui.PushStyleVar(imgui.StyleVar.FramePadding, imgui.ImVec2(1, 3))
				if imgui.Button(fa.ICON_COG.."##1", imgui.ImVec2(21,20)) then
					...
					imgui.OpenPopup(u8"MO | Изменение названия армии")
				end
			imgui.PopStyleVar(1)
			imgui.SameLine(22)
			...
			imgui.PushStyleVar(imgui.StyleVar.FramePadding, imgui.ImVec2(1, 3))
				if imgui.Button(fa.ICON_COG.."##2", imgui.ImVec2(21,20)) then
					...
					imgui.OpenPopup(u8"MO | Изменение названия должности")
				end
			imgui.PopStyleVar(1)
			imgui.SameLine(22)
	...
	imgui.PopItemWidth()
	imgui.EndGroup()
	if imgui.BeginPopupModal(u8"MO | Изменение названия армии", null, imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoMove) then
		imgui.Text(u8"Название армии будет применено к текущему названию")
	...
	if imgui.BeginPopupModal(u8"MO | Изменение названия должности", null, imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoMove) then
...
function imgui.OnDrawFrame()
	if mainWin.v then
		...
		imgui.Begin(fa.ICON_HEARTBEAT .. " DefenseHelper "..scr.version, mainWin, imgui.WindowFlags.NoResize);
			...
			for i,v in ipairs(chgName.rank) do
				...
				local f = io.open(dirml.."/DefenseHelper/MainSetting.med", "w")
				f:write(encodeJson(setting))
				f:flush()
				f:close()
				sampAddChatMessage("{FFFFFF}[{EE4848}DefenseHelper{FFFFFF}]: Настройки сохранены.", 0xEE4848)
				needSave = false
			end
			imgui.PopStyleColor(1)
			imgui.EndGroup()
						...
						if #spur.list ~= 20 then
							for i = 1, 20 do
								if not table.concat(spur.list, "|"):find("Шпаргалка '"..i.."'") then
									...
									spur.text.v = ""
									spurBig.v = false
									local f = io.open(dirml.."/DefenseHelper/Шпаргалки/Шпаргалка '"..i.."'.txt", "w")
									f:write("")
									f:flush()
									f:close()
									break
					...
					--	
						...
						--	imgui.SetCursorPosX(415)
							if imgui.Button(u8"Удалить", imgui.ImVec2(260, 20)) then
								if doesFileExist(dirml.."/DefenseHelper/Шпаргалки/"..spur.list[spur.select_spur]..".txt") then
									os.remove(dirml.."/DefenseHelper/Шпаргалки/"..spur.list[spur.select_spur]..".txt")
								end
							...
							if imgui.Button(u8"Сохранить", imgui.ImVec2(260, 20)) then
								...
								if spur.name.v ~= "" then 
										...
										if doesFileExist(dirml.."/DefenseHelper/Шпаргалки/"..name..".txt") and spur.list[spur.select_spur] ~= name then
											bool = true
								...
								if not bool then
									local f = io.open(dirml.."/DefenseHelper/Шпаргалки/"..name..".txt", "w")
						...
						elseif not spurBig.v and (spur.select_spur >= 1 and spur.select_spur <= 20) then
							...
							if imgui.Button(u8"Редактировать", imgui.ImVec2(170, 20)) then
								spur.edit = true
								local f = io.open(dirml.."/DefenseHelper/Шпаргалки/"..spur.list[spur.select_spur]..".txt", "r")
				...
				imgui.EndChild();
					if cmdBind[selected_cmd].rank <= num_rank.v+1 then
						...
						if imgui.Button(u8"Назначить клавишу", imgui.ImVec2(140, 20)) then 
							imgui.OpenPopup(u8"MO | Установка клавиши для активации");
						...
						if imgui.Button(u8"Очистить активацию", imgui.ImVec2(140, 20)) then 
							...
							cmdBind[selected_cmd].key = {}
								local f = io.open(dirml.."/DefenseHelper/cmdSetting.med", "w")
						...
						imgui.Separator()
							for i,v in ipairs(binder.list) do
								if imgui.Selectable(u8(binder.list[i].name), binder.select_bind == i) then 
									...
									if doesFileExist(dirml.."/DefenseHelper/Binder/bind-"..binder.list[binder.select_bind].name..".txt") then
										local f = io.open(dirml.."/DefenseHelper/Binder/bind-"..binder.list[binder.select_bind].name..".txt", "r")
					...
					--	
						...
						--	imgui.SetCursorPosX(345)
							if imgui.Button(u8"Удалить", imgui.ImVec2(127, 20)) then
								...
								binder.key = {}
								if doesFileExist(dirml.."/DefenseHelper/Binder/bind-"..binder.list[binder.select_bind].name..".txt") then
									os.remove(dirml.."/DefenseHelper/Binder/bind-"..binder.list[binder.select_bind].name..".txt")
								end
								table.remove(binder.list, binder.select_bind) 
								local f = io.open(dirml.."/DefenseHelper/bindSetting.med", "w")
							...
							if imgui.Button(u8"Сохранить", imgui.ImVec2(127, 20)) then
								...
								if not bool then
									...
									local f = io.open(dirml.."/DefenseHelper/bindSetting.med", "w")
									local ftx = io.open(dirml.."/DefenseHelper/Binder/bind-"..binder.list[binder.select_bind].name..".txt", "w")
									f:write(saveJS)
									ftx:write(text)
									f:flush()
									ftx:flush()
			...
			if select_menu[6] then
				...
				imgui.BeginChild("help but", imgui.ImVec2(0,0), true)
					...
					if imgui.IsItemClicked(0) then
						print(shell32.ShellExecuteA(nil, 'open', dirml.."/DefenseHelper/Шпаргалки/", nil, nil, 1))
					end
					--
					imgui.Bullet(); imgui.SameLine()
					...
					imgui.TextColoredRGB("{FF5F29}/mo-delete {FFFFFF} - удалить скрипт")
					--
					imgui.Bullet(); imgui.SameLine()
			...
			if select_menu[7] then
				...
				imgui.BeginChild("about", imgui.ImVec2(0, 0), true)
					...
					imgui.Text(u8"Defens Helper")
					imgui.Spacing()
					imgui.TextWrapped(u8"\tСкрипт был разработан для проекта Ariona Role Play с поддержкой работы на сервере Saint Rose и Glengale для облегчения работы сотрудникам Армии. Благодаря этому приложению Вы получите полный комплекс автоматизации многих действий и наслаждение от пользования.\nОбновления выходят по мере добавления нововведений и исправлений ошибок.")
					imgui.Dummy(imgui.ImVec2(0, 10))
					imgui.Bullet()
					imgui.TextColoredRGB("Разработчик - {FFB700}Jack Cortez aka Harry Cortez")
					imgui.Bullet()
					...
					imgui.TextColoredRGB("Благодарность blast.hk за методическую информацию и его пользователям в решении вопросов.")
						...
						imgui.SameLine()
						imgui.TextColoredRGB("Для связи: VK: {74BAF4}vk.com/https://vk.com/id405621457") --или Discord - {74BAF4}TheVitek#2160")
							if imgui.IsItemHovered() then imgui.SetTooltip(u8"Кликните ЛКМ, чтобы скопировать, или ПКМ, чтобы открыть в браузере")  end
							if imgui.IsItemClicked(0) then setClipboardText("https://vk.com/id405621457") end
							if imgui.IsItemClicked(1) then print(shell32.ShellExecuteA(nil, 'open', 'https://vk.com/id405621457', nil, nil, 1)) end
							imgui.SameLine()
						...
						imgui.TextColoredRGB("Желающие поддержать разработку денюшкой - ".."{68E15D}\"Открыть\"")
							...
							if imgui.IsItemClicked(0) then print(shell32.ShellExecuteA(nil, 'open', 'https://qiwi.me/', nil, nil, 1)) end
						...
						if imgui.Button(u8"Удалить скрипт", imgui.ImVec2(160, 20)) then 
							...
							sampAddChatMessage("", 0xEE4848)
							sampAddChatMessage("", 0xEE4848)
							sampAddChatMessage("{FFFFFF}[{EE4848}DefenseHelper{FFFFFF}]: Внимание! Подтвердите удаление командой {77DF63}/mo-delete.", 0xEE4848)
							mainWin.v = false
						--	sampShowDialog(1002, "{E94C4C}DefenseHelper | {8EE162}Удаление", remove, "Принял", "")
						end
				imgui.EndChild()
			...
			--///Установка клавиши
				...
				if imgui.BeginPopupModal(u8"MO | Установка клавиши для активации", null, imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoMove) then
	...
	--	imgui.SetWindowFontScale(1.1)
		if spur.edit then
				...
				if imgui.Button(u8"Удалить", imgui.ImVec2(357, 20)) then
					...
					table.remove(spur.list, spur.select_spur) 
					spur.select_spur = -1
					if doesFileExist(dirml.."/DefenseHelper/Шпаргалки/"..u8:decode(spur.select_spur)..".txt") then
						os.remove(dirml.."/DefenseHelper/Шпаргалки/"..u8:decode(spur.select_spur)..".txt")
					end
					spur.name.v = ""
					spurBig.v = false
					spur.edit = false
			...
			imgui.BeginChild("spur spec", imgui.ImVec2(1081, 730), true)
				if doesFileExist(dirml.."/DefenseHelper/Шпаргалки/"..spur.list[spur.select_spur]..".txt") then
					for line in io.lines(dirml.."/DefenseHelper/Шпаргалки/"..spur.list[spur.select_spur]..".txt") do
			...
			imgui.BeginGroup()
				...
				if imgui.Button(u8"Начать", imgui.ImVec2(60, 20)) then
					if sobes.selID.v ~= "" then
						...
						sampAddChatMessage("{FFFFFF}[{EE4848}DefenseHelper{FFFFFF}]: Укажите id игрока для начала собеседования.", 0xEE4848)
					end
				end
				...
				imgui.PushStyleColor(imgui.Col.PopupBg, imgui.ImVec4(0.06, 0.06, 0.06, 0.94)) 
					...
					if imgui.BeginPopup("sobQN") then
						...
						if imgui.MenuItem(u8"Выбор больницы") then 
							sampSendChat("Почему Вы выбрали именно нашу организацию для трудоустройства?") 
				...
				imgui.EndChild()
					...
					elseif dep.bool[7] then
						...
						imgui.Spacing()
							...
							if imgui.IsItemClicked(0) then
								print(shell32.ShellExecuteA(nil, 'open', dirml.."/DefenseHelper/Департамент/", nil, nil, 1))
							end
						imgui.Spacing()
						imgui.Spacing()
						if imgui.Button(u8"Подать", imgui.ImVec2(225, 30)) then
							...
							-- 	sampAddChatMessage("{FFFFFF}[{EE4848}DefenseHelper{FFFFFF}]: Gov Новости назначить можно только от 0 до 15 минут.", 0xEE4848)
							-- 	return
							-- end
							
								if doesFileExist(dirml.."/DefenseHelper/Департамент/"..u8:decode(dep.news[dep.newsN.v+1])..".txt") then
								--	print(u8:decode(dep.news[dep.newsN.v+1]))
									for line in io.lines(dirml.."/DefenseHelper/Департамент/"..u8:decode(dep.news[dep.newsN.v+1])..".txt") do
										sampSendChat(line)
										wait(1000)
									end
								end
			...
			if #updinfo < 5 then
				...
				if newversion == scr.version then
					...
					imgui.BeginChild("update log", imgui.ImVec2(0, 0), true)
						if doesFileExist(dirml.."/DefenseHelper/files/update.txt") then
							for line in io.lines(dirml.."/DefenseHelper/files/update.txt") do
...
function sobesRP(id)
	...
	sobes.nextQ = false
		if num_rank.v+1 <= 8 then
			...
			wait(1700)
			sampSendChat("/do В кармане сумки находятся ключи отшкафчиков.")
			wait(1700)
			sampSendChat("/me потянувшись во внутренний карман сумки, "..chsex("достал","достала").." оттуда ключ.")
			wait(1700)
			sampSendChat("/me передал".. chsex("", "а") .." ключ от шкафчика №"..sobes.selID.v.." с формой человеку напротив.")
			wait(1700)
	...
	if thread:status() == "dead" then
		...
		for k, v in pairs(cmdBind) do
			if sKeys == tostring(table.concat(v.key, " ")) then
				...
				elseif k == 16 then
					if resTarg then
						funCMD.kpp(tostring(targID))
					else
						sampSetChatInputEnabled(true)
						sampSetChatInputText("/kpp ")
					end
				end
			end
		end
	else
		sampAddChatMessage("{FFFFFF}[{EE4848}DefenseHelper{FFFFFF}]: В данный момент проигрывается отыгровка.", 0xEE4848)
	end
end
...
local function playBind(tb)
	if not tb.debug.file or #tb.debug.close > 0 then
		if not tb.debug.file then
			sampAddChatMessage("{FFFFFF}[{EE4848}DefenseHelper{FFFFFF}]: Файл с текстом бинда не обнаружен. ", 0xEE4848)
		elseif #tb.debug.close > 0 then
			sampAddChatMessage("{FFFFFF}[{EE4848}DefenseHelper{FFFFFF}]: Диалог, начало которого является строка №"..tb.debug.close[#tb.debug.close]..", не закрыт тегом {dialogEnd}", 0xEE4848)
		end
		addOneOffSound(0, 0, 0, 1058)
		return false
	end
		...
		for k, v in pairs(binder.list) do
			if sKeys == tostring(table.concat(v.key, " ")) then
				thread = lua_thread.create(function()		
					local dir = dirml.."/DefenseHelper/Binder/bind-"..v.name..".txt"	
					local tb = {}
...
function getSpurFile()
	spur.list = {}
    local search, name = findFirstFile("moonloader/DefenseHelper/Шпаргалки/*.txt")
	while search do
...
function getGovFile()
local govls = [[
/gov [Армия ЛС] - Ув.Жители Штата, сегодня в Армия ЛС пройдёт день открытых дверей
/gov [Армия ЛС] - У нас вы получите: лучших сотрудников, быстрый карьерный рост, высокую зарплату
/gov [Армия ЛС] - Ждём всех желающих в Военкомате ЛС.
]]
local govsf = [[
/gov [ВМС] - Ув.Жители Штата, сегодня в ВМС пройдёт день открытых дверей
/gov [ВМС] - У нас вы получите: лучших сотрудников, быстрый карьерный рост, высокую зарплату
/gov [ВМС] - Ждём всех желающих в Военкомате СФ.
]]
local govlv = [[
/gov [Тюрьма ЛВ] - Ув.Жители Штата, сегодня в Тюрьма ЛВ пройдёт день открытых дверей
/gov [Тюрьма ЛВ] - У нас вы получите: лучших сотрудников, быстрый карьерный рост, высокую зарплату
/gov [Тюрьма ЛВ] - Ждём всех желающих в Военкомате ЛВ.
]]
lua_thread.create(function()
	if doesDirectoryExist(dirml.."/DefenseHelper/Департамент/") then
		if doesFileExist(dirml.."/DefenseHelper/Департамент/День открытых дверей.txt") or not doesFileExist(dirml.."/DefenseHelper/Департамент/День открытых дверей ЛСа.txt") then
			os.remove(dirml.."/DefenseHelper/Департамент/День открытых дверей.txt")
			local f = io.open(dirml.."/DefenseHelper/Департамент/День открытых дверей ЛСа.txt", "w")
			f:write(govls)
			f:flush()
			f:close()
			local f = io.open(dirml.."/DefenseHelper/Департамент/День открытых дверей СФа.txt", "w")
			f:write(govsf)
			f:flush()
			f:close()
			local f = io.open(dirml.."/DefenseHelper/Департамент/День открытых дверей ТСР.txt", "w")
			f:write(govlv)
			f:flush()
			f:close()
		end
		dep.news = {}
		local search, name = findFirstFile("moonloader/DefenseHelper/Департамент/*.txt")
		while search do
			if not name then findClose(search) else
...
function tags(par)
		...
		if par:find("{getNickByID:%d+}") then
			for v in par:gmatch("{getNickByID:%d+}") do
				...
				if sampIsPlayerConnected(id) then
					...
					sampAddChatMessage("{FFFFFF}[{EE4848}MO:Ошибка{FFFFFF}]: Параметр {getNickByID:ID} не смог вернуть ник игрока. Возможно игрок не в сети.", 0xEE4848)
					par = par:gsub(v,"")
				end
			end
		end
		...
		if par:find("{getNickByTarget}") then
			if targID ~= nil and targID >= 0 and targID <= 1000 and sampIsPlayerConnected(targID) then
				...
				sampAddChatMessage("{FFFFFF}[{EE4848}MO:Ошибка{FFFFFF}]: Параметр {getNickByTarget} не смог вернуть ник игрока. Возможно Вы не целились на игрока, либо он не в сети.", 0xEE4848)
...
funCMD = {} 
function funCMD.del()
	sampAddChatMessage("{FFFFFF}[{EE4848}DefenseHelper{FFFFFF}]: Вы успешно удалили скрипт.", 0xEE4848)
	sampAddChatMessage("{FFFFFF}[{EE4848}DefenseHelper{FFFFFF}]: Выгрузка скрипта из игры...", 0xEE4848)
	os.remove(scr.path)
	scr:reload()
end
function funCMD.post(stat)
	if not u8:decode(buf_nick.v):find("[а-яА-Я]+%s[а-яА-Я]+") then
		sampAddChatMessage("{FFFFFF}[{EE4848}DefenseHelper{FFFFFF}]: Подождите-ка, сначала нужно заполнить базовую информацию. {90E04E}/mo > Настройки > Основная информация", 0xEE4848)
		return
	end
	if not isCharInModel(PLAYER_PED, 416) then -- not
		sampAddChatMessage("{FFFFFF}[{EE4848}DefenseHelper{FFFFFF}]: Чтобы заступить на мобильный пост, Вам необходимо сначала сесть в карету.", 0xEE4848)
		addOneOffSound(0, 0, 0, 1058)
	else
		...
		elseif bool then
			if stat:find(".+") then
				...
				sampAddChatMessage("{FFFFFF}[{EE4848}DefenseHelper{FFFFFF}]: Укажите обстановку, например, /post Спокойно.", 0xEE4848)
			end
		end
	end
end
function funCMD.warn(text)
	...
	if num_rank.v+1 < 8 then
		sampAddChatMessage("{FFFFFF}[{EE4848}DefenseHelper{FFFFFF}]: Данная команда Вам недоступна. Поменяйте должность в настройках скрипта, если это требуется.", 0xEE4848)
		return
	end
		...
		sampAddChatMessage("{FFFFFF}[{EE4848}DefenseHelper{FFFFFF}]: Используйте команду /+warn [id игрока] [причина].", 0xEE4848)
		end
end
function funCMD.uwarn(id)
	...
	if num_rank.v+1 < 8 then
		...
		sampAddChatMessage("{FFFFFF}[{EE4848}DefenseHelper{FFFFFF}]: Используйте команду /-warn [id игрока].", 0xEE4848)
		end
end
function funCMD.inv(id)
	...
	if num_rank.v+1 < 9 then
		...
		sampAddChatMessage("{FFFFFF}[{EE4848}DefenseHelper{FFFFFF}]: Используйте команду /inv [id игрока].", 0xEE4848)
		end
end
function funCMD.unv(text)
	...
	if num_rank.v+1 < 9 then
		...
		sampAddChatMessage("{FFFFFF}[{EE4848}DefenseHelper{FFFFFF}]: Используйте команду /unv [id игрока] [причина].", 0xEE4848)
		end
end
function funCMD.mute(text)
	...
	if num_rank.v+1 < 8 then
		...
		sampAddChatMessage("{FFFFFF}[{EE4848}DefenseHelper{FFFFFF}]: Используйте команду /+mute [id игрока] [время в минутах] [причина].", 0xEE4848)
		end
end
function funCMD.umute(id)
	...
	if num_rank.v+1 < 8 then
		...
		sampAddChatMessage("{FFFFFF}[{EE4848}DefenseHelper{FFFFFF}]: Используйте команду /-mute [id игрока].", 0xEE4848)
		end
end
function funCMD.rank(text)
	...
	if num_rank.v+1 < 9 then
		...
		thread = lua_thread.create(function()
					sampSendChat("/do В кармане сумки находится футляр с ключами от шкафчиков с формой.")
					wait(1500)
					sampSendChat(chsex("/me потянувшись во внутренний карман сумки, достал".. chsex("", "а") .." оттуда футляр", "/me потянувшись во внутренний карман сумки, достала оттуда футляр"))
					wait(1500)
		...
		sampAddChatMessage("{FFFFFF}[{EE4848}DefenseHelper{FFFFFF}]: Используйте команду /gr [id игрока] [номер ранга].", 0xEE4848)
		end
end
...
function funCMD.openupd()
	print(shell32.ShellExecuteA(nil, 'open', "http://forum.arizona-rp.com/", nil, nil, 1))
end
...
function hook.onSendDialogResponse(id, but, list)
	if sampGetDialogCaption() == ">{FFB300}Посты" then
		if but == 1 then
			...
			sampAddChatMessage("{FFFFFF}[{EE4848}DefenseHelper{FFFFFF}]: На карте была выставлена метка места назначения.", 0xEE4848)
			addOneOffSound(0, 0, 0, 1058)
		elseif but == 0 then
		end
	end
...
function funCMD.kpp(id)
	if thread:status() ~= "dead" then 
		...
		if id:find("(%d+)") then
			thread = lua_thread.create(function()
				sampSendChat(string.format("Приветствую, я %s, сотрудник данного заведения.", u8:decode(buf_nick.v)))
				wait(2000)
				sampSendChat("Предьявите ваши документы, удостоверяющие Вашу личность.")
				wait(2000)
				sampSendChat("А также назовите цель вашего прибытия на территорию базы Тюрьмы ЛВ.")
				wait(2000)
				sampAddChatMessage("{FFFFFF}[{EE4848}DefenseHelper{FFFFFF}]: Нажмите на {23E64A}Enter{FFFFFF} для продолжения.", 0xEE4848)
					while true do
					wait(0)
				...
				sampSendChat("Документы в полном порядке, проезжайте.")
				wait(2000)
				sampSendChat("/me вернул документы законному владельцу, после достал рацию и что-то сказал в неё.")
				wait(2000)
				sampSendChat("/r Пропустил человека с пропуском №"..id..".")
				wait(2000)
			end)
		else
		sampAddChatMessage("{FFFFFF}[{EE4848}DefenseHelper{FFFFFF}]: Используйте команду /kpp [id игрока].", 0xEE4848)
		end
end
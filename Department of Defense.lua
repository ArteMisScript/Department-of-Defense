...
local status = inicfg.load(mainIni, 'Department of Defense.ini')
if not doesFileExist('moonloader/config/Department of Defense.ini') then inicfg.save(mainIni, 'Department of Defense.ini') end
...
imgui.ShowCursor = main_window_state.v
  if main_window_state.v then
	  ...
	  imgui.Begin('Department of Defense | 0.1', main_window_state, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + blocker)		  	
		imgui.BeginChild("##команды", imgui.ImVec2(1050, 35), true, imgui.WindowFlags.NoScrollbar)
			imgui.SetCursorPos(imgui.ImVec2(109, 10))
				imgui.Text(u8"Сокращенные команды: /findihouse - /fh, /findibiz - /fbiz, /house - /h, /fammenu - /fmn |")
			...
			imgui.SetCursorPos(imgui.ImVec2(825, 5))	
				if darktheme.v then					
					if imgui.Button(u8"Dark## DTActive", imgui.ImVec2(50, 25)) then 
						darktheme.v = false
						apply_custom_style()						
					end				
				elseif not darktheme.v then								  
					if imgui.Button(u8"Light## DTunActive", imgui.ImVec2(50, 25)) then 
						darktheme.v = true
						apply_custom_style()
					end	
				end
			...
			imgui.SetCursorPos(imgui.ImVec2(7, 5))
				...
				if imgui.Button(u8"HEAVEN", imgui.ImVec2(90, 25)) then
				   sampAddChatMessage("Семья HEAVEN Empire - Сервер Arizona RP Mesa (07) ", -1)
				end
				imgui.PopStyleColor()
				imgui.PopStyleColor()
				imgui.PopStyleColor()
		...
		imgui.BeginChild("##левый_верх", imgui.ImVec2(300, 120), true, imgui.WindowFlags.NoScrollbar)
	  	...
	  	imgui.BeginChild("##левый_низ", imgui.ImVec2(300, 248), true, imgui.WindowFlags.AlwaysVerticalScrollbar)
				...
				imgui.CheckboxPlus(u8"Реакция на VIP", vip)
				imgui.SameLine()
		...
		imgui.SetCursorPos(imgui.ImVec2(700, 495))
		    	if imgui.Button(u8"О Создателе", imgui.ImVec2(365,28)) then
					...
					sampAddChatMessage("Fyger | blast.hk/threads/49515/", -1)
					sampAddChatMessage("Создатель Автозаточки:", -1)
					sampAddChatMessage("Vadyao | blast.hk/threads/73563/", -1)
					sampAddChatMessage("Создатель VIP Checker:", -1)
					sampAddChatMessage("Cosmo | blast.hk/threads/61249/", -1)
					sampAddChatMessage("Создатель VIP-Resend:", -1)
					sampAddChatMessage("Cosmo | blast.hk/threads/85602/", -1)
					sampAddChatMessage("Улучшено и доработано:", -1)
					sampAddChatMessage("HEAVEN (KitGov4e) | blast.hk/members/414183/", -1)
			   		...
			   		imgui.SetCursorPos(imgui.ImVec2(15, 15))
						...
						if autoad.v then
						    if imgui.CollapsingHeader(u8"Настройки Авто Объявления", true, imgui.TreeNodeFlags.DefaultOpen) then
				                ...
				                if imgui.ButtonActivated(autoad_id_type.v=="0", u8"Обычное## Autoad_id_type 1", imgui.ImVec2(68, 25)) then autoad_id_type.v = "0" end imgui.SameLine()
								if imgui.ButtonActivated(autoad_id_type.v=="1", u8"VIP## Autoad_id_type 2", imgui.ImVec2(68, 25)) then autoad_id_type.v = "1" end
							end
						else
...
function saverconfig() -- Переполнены Upvalues!!!
		   saverconfigNew() -- Загрузка новых переменных
				...
				if inicfg.save(mainIni, 'Department of Defense.ini') then
				 sampAddChatMessage("{0090FF}[Department of Defense {ff981a}] {56d93f} Настройки успешно сохранены!", -1)
				end
end
...
function main()
	...
	sampRegisterChatCommand("mo", function()
	...
	sampAddChatMessage("{0090FF}[Department of Defense {ff981a}] {ffffff}- Загружен! Версия {6495ED}от " ..version.. " ", 0x0090FF)
	autoupdate("https://raw.githubusercontent.com/ArteMisScript/Department-of-Defense/main/version.json", '['..string.upper(thisScript().name)..']: ', "тут ссылка на ваш сайт/url вашего скрипта на форуме (если нет, оставьте как в json)")
    ...
    if #arg > 0 then 
        local k = calc(arg)
        if k then 
            sampAddChatMessage('{01A0E9}[Department of Defense] {ff981a}Решенный пример: ' ..arg.. ' = ' .. k,0x6495ED)  
        end 
    else sampAddChatMessage("{01A0E9}[Department of Defense] {ff981a}Введите пример который нужно решить" , 0x6495ED)
    end
    --if #arg > 0 and arg:find("euro") then 
        --local k = 222
        --if k then 
           -- sampAddChatMessage('{01A0E9}[ARZ Helper] {ff981a}Решенный пример: ' ..arg.. ' = ' .. k,0x6495ED)  
        --end 
    --else sampAddChatMessage("{01A0E9}[ARZ Helper] {ff981a}Введите пример который нужно решить" , 0x6495ED)
    --end
    end)
	  ...
	  if infotextb.v or infotextp.v then
        ...
        if checkpos_vr then
            ...
            posX_vr, posY_vr = getCursorPos()
			        testmessage_vs = "Test message. Vip-Resend by Cosmo. Settings by Heaven (KitGov4e)."
		...
		if binder.v then
		   if bindersetkey.v then
           	if wasKeyPressed(0xBD) and isKeyDown(0x11) and not sampIsChatInputActive() and not sampIsDialogActive() and not isPauseMenuActive() and not isSampfuncsConsoleActive() then 
                ...
                sampAddChatMessage(' {0090FF}[Department of Defense {ff981a}(BINDER){0090FF}] {56d93f} Установлен пресет #'..bindtab.v..'', -1)						
			end
           else			
				  ...
				  idf = text:match('%d+')
				   wait(100)
				    if sampIsPlayerConnected(tonumber(idf)) then
		              ...
		              famfile:close()
		              sampAddChatMessage('{F2A1EE}[MO Fam Helper] {ffffff}'..sampGetPlayerNickname(idf)..' {ffff00}('..sampGetPlayerScore(idf)..' lvl) {42b201}Был добавлен в ваш список!', -1)			
				    end		
				end)	
		    end				
		end		
...
function hook.onShowDialog(dialogId, dialogStyle, dialogTitle, okButtonText, cancelButtonText, dialogText)
	...
	if sctime_toggle then --(Server Time by Kalashnikov)	
...
function fastreport(text)
	if fastrep.v then
		if #text == 0 then
			sampAddChatMessage('{ff6347}[MO Быстрый репорт]:{ffffff} Введите ваш репорт', 0xff6347)
		else
			sampSendChat('/rep')
...
function update()
	sampAddChatMessage("Следи за обновлениями в теме скрипта | blast.hk/threads/87731/",-1)
	sampAddChatMessage("Или в профиле: KitGov4e | blast.hk/members/414183/",-1)
end
